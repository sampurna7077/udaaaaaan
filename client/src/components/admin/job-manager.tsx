...
const jobSchema = z.object({
  ...
  // include featured so it is not stripped by the schema
  featured: z.boolean().default(false),
  // include status so it is not stripped and can be persisted
  status: z.enum(["draft", "published", "archived"]).default("published").optional(),
  vacancies: z.number().min(1, "Vacancies must be at least 1").default(1)
});
...
export default function JobManager() {
  ...
  const createJobMutation = useMutation({
    mutationFn: async (data: z.infer<typeof jobFormSchema>) => {
      ...
      if ((jobData as any).tags) {
        jobData.tags = JSON.stringify(String((jobData as any).tags).split(',').map((tag: string) => tag.trim()));
      }
      if ((jobData as any).vacancies === undefined || jobData.vacancies === null) {
        jobData.vacancies = 1;
      } else {
        jobData.vacancies = Number(jobData.vacancies) || 1;
      }
  ...
  const updateJobMutation = useMutation({
    mutationFn: async (data: z.infer<typeof jobFormSchema>) => {
      ...
      if ((jobData as any).vacancies !== undefined) {
        jobData.vacancies = Number(jobData.vacancies) || 1;
      }
  ...
  const handleEditJob = (job: Job) => {
    ...
    jobForm.reset({
      ...
      vacancies: Number((job as any).vacancies) || 1,
