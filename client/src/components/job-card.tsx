...
import { Heart, MapPin, Clock, Building2, ExternalLink, Award, DollarSign, Users, Briefcase, Eye, X, Calendar } from "lucide-react";
...
export default function JobCard({ job, showFullDetails = false }: JobCardProps) {
  ...
  const handleViewDetails = () => {
    // Open a detailed view dialog with full job information
  ...
  const getRemoteTypeBadge = () => {
    ...
    const key = (job.remote_type || job.remoteType || '').toLowerCase();
    return variants[key] || "bg-slate-100 text-slate-800";
  };
  ...
  const openingsCount = Number((job as any).vacancies ?? 0);
  const remoteRaw: string = (job.remote_type || (job as any).remoteType || '').toString();
  const remoteLabel: string = remoteRaw
    ? (remoteRaw === 'onsite' ? 'On-Site' : remoteRaw.charAt(0).toUpperCase() + remoteRaw.slice(1))
    : 'Remote';
  const expRaw: string = ((job as any).experience_level || (job as any).experienceLevel || '').toString();
  const expLabel: string = expRaw ? expRaw.charAt(0).toUpperCase() + expRaw.slice(1) + ' Level' : '';
  const visaSupported: boolean = Boolean((job as any).visa_support ?? (job as any).visaSupport);
  const jobTypeText: string = (((job as any).jobType || '') as string)
    .replace('-', ' ')
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="flex items-center text-xs md:text-sm text-slate-500 mt-1 md:mt-2 flex-wrap gap-1">
                      ...
                      {openingsCount > 0 && (
                        <>
                          <span className="mx-2 flex-shrink-0">•</span>
                          <span className="flex-shrink-0" data-testid="job-vacancies">{openingsCount} {openingsCount === 1 ? 'opening' : 'openings'}</span>
                        </>
                      )}
                    </div>
                  </div>
              ...
              <div className="md:text-right md:mr-4">
                {salaryInfo && (
                  <div className="font-bold text-lg md:text-xl lg:text-lg text-slate-800 md:whitespace-nowrap mb-2 md:mb-0" data-testid="job-salary">
                    ...
                    <span className="text-sm text-slate-500 ml-1 block md:inline">{salaryInfo.periodText}</span>
                  </div>
                )}
              </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            ...
                            <p className="font-medium">{remoteLabel}</p>
                          </div>
                        </div>
                        ...
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            ...
                            <p className="font-medium">{jobTypeText}</p>
                          </div>
                        </div>
                          ...
                          <div className="flex items-center space-x-3">
                            ...
                            <div>
                              ...
                              <p className="font-medium">
                                {salaryInfo.salaryText} {salaryInfo.periodText}
                              </p>
                            </div>
                          </div>
                        )}
                        ...
                        {openingsCount > 0 && (
                          <div className="flex items-center space-x-3">
                            ...
                            <div>
                              ...
                              <p className="font-medium">{openingsCount} opening{openingsCount === 1 ? '' : 's'}</p>
                            </div>
                          </div>
                        )}
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
            <Button
              ...
              data-testid="button-view-details-featured"
            >
              View Details
            </Button>
            <Button
        ...
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="mt-6 grid md:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              <div>
                <h4 className="font-semibold text-slate-800 mb-2">Job Details</h4>
                <div className="space-y-3">
                  ...
                  {salaryInfo && (
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        <p className="text-sm text-slate-500">Salary</p>
                        <p className="font-medium">{salaryInfo.salaryText} {salaryInfo.periodText}</p>
                      </div>
                    </div>
                  )}
                  <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  <Badge className={getRemoteTypeBadge()}>{remoteLabel}</Badge>
                  {visaSupported && (
                    <Badge className="bg-coral-100 text-coral-800">Visa Support</Badge>
                  )}
                  {!!expLabel && (
                    <Badge className="bg-amber-100 text-amber-800">{expLabel}</Badge>
                  )}
                  {job.category && (
                    <Badge variant="secondary">{job.category}</Badge>
                  )}
