...
export function registerRoutes(app: Express): Server {
  ...
  app.get("/api/jobs", 
    ...
    try {
      ...
      const filters = {
        ...
        excludeCountry: excludeCountry as string,
      };
  ...
  // Featured jobs endpoint - unlimited for carousel
  app.get("/api/jobs/featured", 
    validateInput([validationRules.pagination]), 
    async (req, res) => {
    try {
      // Minimal cache headers for instant updates
      res.set({
        'Cache-Control': 'public, max-age=10', // Only 10 seconds cache for instant updates
      });

      const jobs = await storage.getJobs({ 
        isFeatured: true, 
        isPublished: true
        // No limit - show all featured jobs for carousel
      });
      res.json(jobs);
    } catch (error) {
  ...
  app.post("/api/jobs/:id/apply", 
    ...
    try {
      ...
      // After successful application, decrement vacancies if available
      try {
        const freshJob = await storage.getJob(jobId);
        const currentVacancies = parseInt((freshJob?.vacancies ?? freshJob?.vacancies) as any) || 0;
        if (currentVacancies > 0) {
          await storage.updateJob(jobId, { vacancies: currentVacancies - 1 });
        }
      } catch (e) {
  ...
  app.get("/api/testimonials", 
    ...
    async (req, res) => {
    try {
      // Minimal cache headers for fast updates
      res.set({
        'Cache-Control': 'public, max-age=120', // Only 2 minutes cache
      });

      const testimonials = await storage.getTestimonials({ isApproved: true });
  ...
  app.get("/api/resources", 
    ...
    try {
      ...
      let { type, category, country, limit, isFeatured } = req.query as Record<string, string>;
      // Normalize plural aliases used by client UI
      const typeNormalized = type === 'guides' ? 'guide' : type === 'downloads' ? 'download' : type;

      let resources = await storage.getResources({
        type: typeNormalized as string,
        category: category as string,
        country: country as string,
        isPublished: true,
      ...
      // Expand FAQ resources with faqItems into individual Q/A entries for public UI
      if (typeNormalized === 'faq') {
        const expanded: any[] = [];
        for (const resItem of resources as any[]) {
          if (Array.isArray((resItem as any).faqItems) && (resItem as any).faqItems.length > 0) {
            (resItem as any).faqItems.forEach((faq: any, idx: number) => {
              expanded.push({
                ...resItem,
                id: `${resItem.id}-${idx + 1}`,
                title: faq.question,
  ...
  app.put("/api/admin/jobs/:id", 
    ...
    try {
      ...
      // normalize potential featured/status values coming as strings
  ...
  // NOTE: Duplicate public resources route was present; consolidated above version with validation and normalization
    ...
    validateInput([
      ...
      body('content').custom((value, { req }) => {
        ...
        if (req.body.type === 'faq') {
          return true; // No content required for FAQ (even on publish)
        }
        // Only require content when publishing guide/blog; allow empty drafts
  ...
  app.get("/api/stats", 
    ...
    try {
      // Minimal cache headers for fast updates
      res.set({
        'Cache-Control': 'public, max-age=300', // Only 5 minutes cache for stats
      });

      const [jobsData, testimonials, companies] = await Promise.all([
        storage.getJobs({ limit: 1000, offset: 0 }),
        storage.getTestimonials({ isApproved: true }),
        storage.getCompanies()
      ]);
  ...
  app.post("/api/admin/advertisements", 
    ...
    async (req: any, res) => {
    try {
      // Coerce strings to booleans/numbers as needed
      const body = { ...req.body };
      if (typeof body.isActive === 'string') body.isActive = body.isActive === 'true';
      if (typeof body.priority === 'string') body.priority = parseInt(body.priority) || 0;

      // Normalize file path if it's an upload URL
      if (body.filePath && body.filePath.includes('storage.googleapis.com')) {
        const { ObjectStorageService } = await import("./objectStorage");
        const objectStorageService = new ObjectStorageService();
        body.filePath = objectStorageService.normalizeAdvertisementPath(body.filePath);
      }
  ...
  app.put("/api/admin/advertisements/:id", 
    ...
    async (req: any, res) => {
    try {
      // Coerce and convert string dates/booleans to proper types
      const updateData = { ...req.body };
