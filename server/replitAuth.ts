...
// Use JSON storage for authentication
...
export function getSession() {
  ...
  const sessionStore = new MemoryStoreConstructor({
    checkPeriod: sessionTtl, // Prune expired sessions every week
    ttl: sessionTtl,
  });
  return session({
...
async function upsertUser(
  ...
  await storage.upsertUser({
    id: claims["sub"],
    username: claims["email"] || claims["sub"], // Use email as username or fallback to sub
    email: claims["email"],
...
export async function setupAuth(app: Express) {
  ...
  const verify: VerifyFunction = async (
    ...
    const user = {
      ...
      username: claims.email as string || claims.sub as string, // Use email as username or fallback to sub
