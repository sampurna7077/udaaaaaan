...
var DEFAULT_JSON_DB_FOLDER, JSON_DB_FOLDER, SimpleJSONStorage, simpleJsonDb;
var init_simple_json_storage = __esm({
  "server/simple-json-storage.ts"() {
    "use strict";
    DEFAULT_JSON_DB_FOLDER = path.resolve(
      process.cwd(),
      "06jbjz5PvFtImMG2VzFz6LHiD0Uom5E9ltYCAfKtS2sZAXCixewC4BfqBHBha5HUh328YEL2BBHEKpTe"
    );
    JSON_DB_FOLDER = process.env.JSON_DB_FOLDER || DEFAULT_JSON_DB_FOLDER;
...
var init_json_storage_adapter = __esm({
  "server/json-storage-adapter.ts"() {
    ...
    JSONStorageAdapter = class {
      ...
      mapAdvertisementToDb(input) {
        if (!input) return input;
        const mapped = { ...input };
        if (mapped.isActive !== void 0) mapped.is_active = mapped.isActive;
        if (mapped.clickCount !== void 0) mapped.click_count = mapped.clickCount;
        if (mapped.impressionCount !== void 0) mapped.impression_count = mapped.impressionCount;
        if (mapped.startDate !== void 0) mapped.start_date = mapped.startDate;
        if (mapped.endDate !== void 0) mapped.end_date = mapped.endDate;
        if (mapped.filePath !== void 0) mapped.file_path = mapped.filePath;
        if (mapped.fileType !== void 0) mapped.file_type = mapped.fileType;
        return mapped;
      }
      mapResourceToDb(input) {
        if (!input) return input;
        const mapped = { ...input };
        if (mapped.isPublished !== void 0) mapped.is_published = mapped.isPublished;
        if (mapped.isFeatured !== void 0) mapped.is_featured = mapped.isFeatured;
        if (mapped.publishedAt !== void 0) mapped.published_at = mapped.publishedAt;
        return mapped;
      }
      // Method to clear cache for immediate data refresh
      clearCache(collection) {
      ...
      async getResources(filters = {}) {
        ...
        if (filters.isFeatured !== void 0) {
          query3.is_featured = filters.isFeatured;
        }
      ...
      async createResource(resource) {
        ...
        const created = await simpleJsonDb.create("resources", {
          ...payload,
          id: payload.id || nanoid2(),
          slug: payload.slug || payload.title?.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, ""),
          is_published: payload.is_published !== void 0 ? payload.is_published : true,
          is_featured: payload.is_featured !== void 0 ? payload.is_featured : false,
          published_at: payload.published_at || (/* @__PURE__ */ new Date()).toISOString()
        });
...
function setupAuth(app2) {
  const sessionSettings = {
    ...
    cookie: {
      ...
      httpOnly: true,
      secure: process.env.SESSION_COOKIE_SECURE ? process.env.SESSION_COOKIE_SECURE === "true" : process.env.NODE_ENV === "production",
...
var validationRules = {
  ...
  jobFilters: [
    ...
    query("excludeCountry").optional().trim().escape().isLength({ max: 50 }),
...
function registerRoutes(app2) {
  ...
  app2.get(
    ...
    async (req, res) => {
      try {
        ...
        const jobs = await storage3.getJobs({
          isFeatured: true,
          isPublished: true,
          limit: 6
        });
  ...
  app2.post(
    ...
    async (req, res) => {
      try {
        ...
        try {
          const freshJob = await storage3.getJob(jobId);
          const currentVacancies = parseInt(freshJob?.vacancies ?? freshJob?.vacancies) || 0;
          if (currentVacancies > 0) {
            await storage3.updateJob(jobId, { vacancies: currentVacancies - 1 });
          }
        } catch (e) {
          console.error("Vacancies decrement failed for job:", jobId, e);
        }
    ...
    async (req, res) => {
      try {
        res.set({
          ...
          // Only 2 minutes cache
        });
        const testimonials = await storage3.getTestimonials({ isApproved: true });
  ...
  app2.get(
    ...
    validateInput([
      ...
      query2("isFeatured").optional().isBoolean().withMessage("isFeatured must be true or false")
    ]),
    async (req, res) => {
      try {
        res.set({
        ...
        let { type, category, country, limit, isFeatured } = req.query;
        const typeNormalized = type === "guides" ? "guide" : type === "downloads" ? "download" : type;
        let resources = await storage3.getResources({
          type: typeNormalized,
          category,
          country,
          isPublished: true,
          isFeatured: isFeatured === "true" ? true : void 0,
          limit: limit ? parseInt(limit) : void 0
        });
        if (typeNormalized === "faq") {
          const expanded = [];
          for (const resItem of resources) {
            if (Array.isArray(resItem.faqItems) && resItem.faqItems.length > 0) {
              resItem.faqItems.forEach((faq, idx) => {
                expanded.push({
                  ...resItem,
                  id: `${resItem.id}-${idx + 1}`,
                  title: faq.question,
  ...
  app2.post(
    ...
    validateInput([
      ...
      body2("featured").optional().isBoolean().withMessage("Featured must be boolean"),
      body2("status").optional().isIn(["draft", "published", "archived"]).withMessage("Invalid status"),
  ...
  app2.put(
    ...
    async (req, res) => {
      try {
        ...
        if (typeof updateData.visaSupport === "string") {
          updateData.visaSupport = updateData.visaSupport === "true";
        }
        const job = await storage3.updateJob(req.params.id, updateData);
        res.json(job);
      } catch (error) {
    ...
    async (req, res) => {
      try {
        ...
        const resources = await storage3.getResources({
          type: typeNormalized,
          category,
          // Pass undefined to include all (both drafts and published)
          isPublished: void 0
        });
        res.json(resources);
      } catch (error) {
  ...
  app2.post(
    ...
    validateInput([
      ...
      body2("content").custom((value, { req }) => {
        ...
        if (isPublishing && (req.body.type === "guide" || req.body.type === "blog") && (!value || value.trim().length < 10)) {
          throw new Error("Content must be at least 10 characters when publishing guides and blogs");
        }
        return true;
      }),
    ...
    async (req, res) => {
      try {
        ...
        if (typeof bodyData.isPublished === "string") {
          bodyData.isPublished = bodyData.isPublished === "true";
        }
  ...
  app2.put(
    ...
    async (req, res) => {
      try {
        ...
        const resource = await storage3.updateResource(req.params.id, updateData);
        res.json(resource);
      } catch (error) {
    ...
    async (req, res) => {
      try {
        res.set({
          ...
          // Only 5 minutes cache for stats
        });
        const [jobsData, testimonials, companies] = await Promise.all([
          storage3.getJobs({ limit: 1e3, offset: 0 }),
          storage3.getTestimonials({ isApproved: true }),
          storage3.getCompanies()
        ]);
        const stats = {
  ...
  app2.post(
    ...
    async (req, res) => {
      try {
        ...
        if (typeof body3.isActive === "string") body3.isActive = body3.isActive === "true";
        if (typeof body3.priority === "string") body3.priority = parseInt(body3.priority) || 0;
        if (body3.filePath && body3.filePath.includes("storage.googleapis.com")) {
          ...
          body3.filePath = objectStorageService.normalizeAdvertisementPath(body3.filePath);
        }
        const requestData = { ...body3 };
  ...
  app2.put(
    ...
    async (req, res) => {
      try {
        ...
        if (typeof updateData.priority === "string") updateData.priority = parseInt(updateData.priority) || 0;
